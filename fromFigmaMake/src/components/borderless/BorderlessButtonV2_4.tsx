...
interface BorderlessButtonV2_4Props {
  icon: LucideIcon;
...
// ==================== V2.4: Synced Background Animation + Light Background ====================
// Fix: Background layer also animates with brightness + scale
// Page: Light background (slate-50 → white)

export function BorderlessButtonV2_4({
  icon: Icon,
  color = { r: 6, g: 182, b: 212 },
  size = 'md',
  onClick,
  className = "",
}: BorderlessButtonV2_4Props) {
  const [isHovered, setIsHovered] = useState(false);
  const [isPressed, setIsPressed] = useState(false);  // ⭐ Shared state
  ...
  return (
    ...
    >
      {/* Layer 1: CodePen Mesh Gradient Border */}
      <div
        style={{
          position: 'absolute',
          inset: 0,
      ...
      {/* Layer 3: Colored background - ⭐⭐⭐ NOW SYNCED WITH PRESS STATE */}
      <motion.div
        animate={{
          // Hover animations (slow, 1s)
          filter: isHovered ? 'blur(0px)' : `blur(${iconBackgroundBlur}px)`,
          boxShadow: isHovered
            ? `0 0 20px rgba(${r},${g},${b},0.35)`
            : `0 0 30px rgba(${r},${g},${b},0.25)`,
          
          // ⭐⭐⭐ NEW: Press animations (fast, 0.2s) - synced with button layer
          brightness: isPressed ? 1.5 : 1.0,
      ...
      <motion.button
        ...
        onMouseDown={() => {
          setIsPressed(true);
          console.log('Button pressed (v2.4 - synced)!');
        }}
        onMouseUp={() => {
          setIsPressed(false);
          console.log('Button released (v2.4 - synced)!');
        }}
        animate={{
